...
import { isProd, isDev, GREY_NEUTRAL } from "c:/constants"
...
export default (state, emit) => {
  ...
  if (!activeTestBlock || isProd) return
  switch (activeTestId) {
    case "after-image":
      return html`
        <p class="p-absolute pos-tl u-flex u-flex--stack">
          <span class="text-secondary"
            >test name:<mark>${activeTestBlock.TEST_NAME}</mark></span
          >
          <span class="text-secondary"
            >test state:<mark
              >${activeTestBlock.stateMachine}</mark
            ></span
          >
