...
module.exports = function() {
  ...
  function calculateTouchPoint(evt, pos) {
    ...
    const {
      ...
      startTime,
      endTime,
    } = updatePosOnDragXY(evt)
    if (_options.onMove) {
      _options.onMove({ x, y, pos })
    }
    const { x: trueX, y: trueY } = getNormalizedXY(evt)
    let newX
    let newY
    if (_previousXY) {
    ...
    touchesPayload[1] = r
    touchesPayload[2] = Math.floor(red)
    touchesPayload[3] = Math.floor(green)
    touchesPayload[4] = Math.floor(blue)
    touchesPayload[5] = newX
    touchesPayload[6] = newY
    touchesPayload[7] = [startTime, endTime]
    touchesPayload[8] = xy2polar(trueX, trueY)
