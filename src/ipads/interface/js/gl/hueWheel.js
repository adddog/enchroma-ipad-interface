  ...
  return regl({
    ...
    frag: `
      ...
      void main () {
        ...
        float affectedBrightness = clamp(brightness + r, 0., 1.);
        // vec3 color = texture2D(texture, uv).rgb;
        vec3 circleColor = vec3(circle(vUv,1.0));
        vec3 color = circleColor * vec3(hsv2rgb(vec3(deg, 0.5, brightness))) + ((1. - circleColor) * baseColor);
        gl_FragColor = vec4(color,1);
        //gl_FragColor = vec4(vec3(affectedBrightness),1);
        //gl_FragColor = vec4(vec3(clamp(r, 0., 1.)),1);
      }`,
